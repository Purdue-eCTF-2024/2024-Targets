...
// Data type for receiving a verification message  
typedef struct {
 ...
 * 
 * @param address: i2c_addr_t, I2C i2c_addr_t of i2c_addr_t
...
int secure_receive(i2c_addr_t address, uint8_t* buffer) {
    ...
    uint8_t lengthReceived = poll_and_receive_packet(address, buffer);
    for (int i = 0; i < lengthReceived; i++) {
        buffer[i] = buffer[i] ^ MESSAGE_KEY;
    }
// uint8_t* outPut = buffer;
// // the key_128 is same all across
// uint32_t tempArray [4] = {HASH_SECRET1, HASH_SECRET2, HASH_SECRET3, HASH_SECRET4};
// uint8_t* key_128;
// key_128 = (uint8_t*)tempArray;
...
void init() {
    ...
    if (flash_status.flash_magic != FLASH_MAGIC) {
        ...
        // ID[COMPONENT_CNT] = {COMPONENT_IDS};
...
int key_sync() {
    ...
    // Send key verification command to each component
    for (unsigned i = 0; i < flash_status.component_cnt; i++) {
        ...
        command->opcode = COMPONENT_CMD_VERIFICATION;
        //command->params[0] = globalKey;

        // Send out command and receieve result
        int len = issue_cmd(addr, transmit_buffer, receive_buffer);
        if (len == ERROR_RETURN) {
            print_error("Could not validate key\n");
            return ERROR_RETURN;
        }

        verification_message *verification = (verification_message *)receive_buffer;

        // Check that the result is correct
        if (verification->component_id != flash_status.component_ids[i] || verification->verification_key != verificationKey) {
            print_error("Verification with Component ID: 0x%08x was invalid\n",
...
void boot() {
    ...
    while (currentState == 0) {
        currentState = key_sync();
        // // TEST for SR 5
        // char name[] = "Junhyung Park";
        
        // // Set the I2C address of the component
        // i2c_addr_t addrTest = component_id_to_i2c_addr(flash_status.component_ids[0]);

        // int size = secure_send (addrTest, (uint8_t*)name, strlen(name));

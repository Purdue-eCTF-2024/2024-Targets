...
int validate_components() {
    ...
    for (unsigned i = 0; i < flash_status.component_cnt; i++) {
        ...
        // print_info("Issuing command for 0x%08x", flash_status.component_ids[i]);
        ...
        if (validate->component_id != flash_status.component_ids[i] || strcmp((char*)validate->password, passwordHash)) {
            //print_info("a=%s\n", (char*)validate->password);
            //print_info("b=%s\n", passwordHash);
            return ERROR_RETURN;
        }
    }
